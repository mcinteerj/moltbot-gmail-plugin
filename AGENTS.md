...
- Account keys are email addresses (e.g. `"user@example.com"`). The gateway's routing layer normalizes these (e.g. to `"user-example-com"`), so `resolveGmailAccount` handles reverse-matching.
